...
uint64_t get_elapsed_sleep(long sec, long nsec){
  //measure clocks before and after nanosleep(), and use the difference to find out clocks elapsed.
  uint64_t bef, aft;
  get_clocks(bef);
...
uint64_t get_elapsed_busywait(long sec, long nsec){
  /*get clocks in bef. place current time in start timespec. place
    time in args in end timespec. add start into end to get goal
    timespec. enter while loop and continuously update start time
    until start is in the future compared to end, exit loop, and get
    clocks in aft. return the difference*/
  uint64_t bef, aft;
