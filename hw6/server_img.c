...
int worker_main(void *args){
	...
	while(!targs->worker_done | (queue->size!=0)){
		...
		switch(opcode){
		case IMG_ROT90CLKW:
			postop=rotate90Clockwise(img_arr[curreq.req.img_id],NULL);
			break;
		case IMG_BLUR:
			postop=blurImage(img_arr[curreq.req.img_id]);
			break;
		case IMG_SHARPEN:
			postop=sharpenImage(img_arr[curreq.req.img_id]);
			break;
		case IMG_VERTEDGES:
			postop=detectVerticalEdges(img_arr[curreq.req.img_id]);
			break;
		case IMG_HORIZEDGES:
			postop=detectHorizontalEdges(img_arr[curreq.req.img_id]);
			break;
		case IMG_RETRIEVE:
